...
$(function () {
    ...
    //根据用户登录状态决定显示的功能接口，是startButton函数的先决条件
    starButton();
...
function comment() {
    ...
    $(".main").delegate(".pub", "click", function () {
        ...
        $.ajax({
            url: "http://123.56.15.54:8080/anbbs/reply",
            // dataType: "json",
            method: "post",
            async: true,
            data: {
    ...
    $("#btn-cmt").on("click", function () {
        ...
        if(content !== ""){
            //ajax
            $.ajax({
                url: "http://123.56.15.54:8080/anbbs/comment",
                method: "post",
                dataType: "json",
...
function starButton() {
    ...
    if(sessionStorage.user){
        ...
        $.ajax({
            url: "http://123.56.15.54:8080/anbbs/star/check/" + user.id + "/" + getRequest("id"),
            method: "post",
            dataType: "json",
            success: function (res) {
                if(res.status){
        ...
        $('#star-btn').click(function () {
            ...
            $.ajax({
                url: "http://123.56.15.54:8080/anbbs/favorite/user/" + user.id,
                method: "get",
                dataType: "json",
                success: function (res) {
            ...
            $('#my-prompt-star').modal({
                ...
                onConfirm: function (e) {
                    ...
                    $.ajax({
                        url: "http://123.56.15.54:8080/anbbs/star",
                        method: "post",
                        dataType: "json",
                        data: {
                            "favoriteId": e.data,
        ...
        $("#unstar-btn").click(function () {
            ...
            $('#my-confirm-unstar').modal({
                ...
                onConfirm: function() {
                    // todo
                    $.ajax({
                        url: "http://123.56.15.54:8080/anbbs/star/id/" + tid,
                        dataType: "json",
                        type: "delete",
                        //method : "delete", !!!
                        success: function () {
...
function getPostInfo() {
    $.ajax({
        url: "http://123.56.15.54:8080/anbbs/post/id/" + getRequest("id"),
        method: "get",
        dataType: "json",
        success: function (res) {
            if(res.status){
...
function genCmt(cid, uid, uimg, nickname, time ,content) {
    return "<article class=am-comment id=" + cid + ">" +
        ...
        "                                    评论于 <time>" + time + "</time> \n" +
        "                                    <a href='login.html' class=reptag>回复</a>\n" +
        "                                </div>\n" +
        "                            </header>\n" +
...
function getHot() {
    $.ajax({
        url: 'http://123.56.15.54:8080/anbbs/post/hot',
        dataType: 'json',
        async: true,
        type: 'get',
